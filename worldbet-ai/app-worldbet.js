...
class WorldBetAI {
  ...
  getBankroll() {
    if (this.auth.isLoggedIn && this.auth.profile?.bankroll != null) {
      return parseFloat(this.auth.profile.bankroll) || DEFAULT_BANKROLL;
    }
    return parseFloat(this.config.bankroll) || DEFAULT_BANKROLL;
  }
  ...
  updateBankrollDisplay() {
    ...
    if (cfg) cfg.textContent = formatMoney(amount);
    const modalBr = document.getElementById('bet-bankroll');
    if (modalBr) modalBr.textContent = formatMoney(amount);
  }
    ...
    if (this.auth.isLoggedIn) {
      return `<div class="card" id="settings-auth-card">
        ...
        <p style="font-size:var(--text-sm);color:var(--color-text-muted);margin-bottom:var(--space-3)">
          Bankroll virtual: <span class="rec-odds-big" id="cfg-bankroll-display">${formatMoney(this.getBankroll())}</span>
        ...
        <div class="modal" style="max-width:420px">
          ...
          <div class="modal-body">
            ...
            <div id="auth-panel-register" style="display:none">
              ...
              <p style="font-size:var(--text-xs);color:var(--color-text-muted);margin-top:var(--space-3)">Bankroll inicial: $1.000.000 virtual</p>
            </div>
  ...
  renderRecHero(rec, fixture) {
    ...
    return `<div class="rec-hero">
      ...
      <div class="rec-hero-meta">
        ...
        ${kelly ? `<span>Stake <strong>${formatMoney(kelly.stakeSuggestion)}</strong></span>` : ''}
        ${gain > 0 ? `<span>Ganancia <strong>+${formatMoney(gain)}</strong></span>` : ''}
      </div>
  ...
  renderAnalysisPickRow(v) {
    return `<div class="analysis-pick-row" style="padding:var(--space-3);background:var(--color-surface-offset);border-radius:var(--radius-md);margin-bottom:var(--space-2)">
      ...
      <span style="font-size:var(--text-sm)">@ ${v.odds.toFixed(2)} · Prob. modelo: ${pct(v.prob)} · Kelly 1/4: ${pct(v.kelly.recommendedBet)} (${formatMoney(v.kelly.stakeSuggestion)})</span>
    </div>`;
  }
  ...
  renderRecBox(rec, compact) {
    ...
    return `<div class="rec-box ${confCls}">
      ...
      <div class="rec-grid">
        ...
        ${rec.primaryKelly ? `<div><span class="rec-k">Kelly 1/4</span><br>${formatMoney(rec.primaryKelly.stakeSuggestion)}</div>` : ''}
      </div>
  ...
  renderUserBetRow(b) {
    ...
    const pl = b.status === 'won' ? `+${formatMoney(b.payout)}` : b.status === 'lost' ? `-${formatMoney(b.stake)}` : '—';
    ...
    return {
      ...
      mobileCard: `<div class="bet-card">
        ...
        <div class="bet-card-row"><span>Stake</span><span>${formatMoney(b.stake)}</span></div>
  ...
  renderUserBetsSection({ compact = false } = {}) {
    ...
    const kpiGrid = compact
      ? `<div class="kpi-grid user-bets-kpi-compact" style="margin-bottom:var(--space-4)">
          <div class="kpi-card"><div class="kpi-label">Bankroll</div><div class="kpi-value">${formatMoney(this.auth.profile?.bankroll ?? this.config.bankroll)}</div></div>
  ...
  renderSettings() {
    return `
      ...
      <div class="card settings-form">
        ...
        <div class="form-group"><label>Bankroll virtual (COP)</label>
          <p id="cfg-bankroll-display" class="bankroll-chip" style="display:inline-flex">${formatMoney(this.getBankroll())}</p>
  ...
  async openMatchModal(matchId) {
      ...
      <div class="modal-overlay" id="modal-overlay" role="dialog" aria-modal="true" aria-label="Análisis de partido">
        <div class="modal">
          ...
          <div class="modal-body">
            ...
            <div class="filters">
              <span class="bankroll-chip" id="bet-bankroll" aria-label="Bankroll">${formatMoney(this.getBankroll())}</span>
              ...
              <input type="number" id="bet-stake" step="1" min="1" placeholder="Stake COP" aria-label="Stake">
            </div>
    ...
    const updateBet = () => {
      ...
      document.getElementById('bet-result').textContent =
        `Apostar: ${formatMoney(stake)} | Ganancia potencial: +${formatMoney(gain)} | Riesgo: ${kelly.riskLevel}`;
    };
